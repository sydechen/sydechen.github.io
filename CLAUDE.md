...
Sydney Chen's portfolio site. Plain static HTML served by GitHub Pages from `main` — no build step, no framework, no dependencies. Each page is self-contained: markup plus an inline `<style>` block using the design tokens declared at the top of `index.html`.
